...
class mp(object):
    ...
    def ransacmp(self,i):
        self.idx_final_y = []
        if(len(self.idx[i][0])>1):
            # run ransac per slice to find the linear increasing line
            ransac = RANSACRegressor(residual_threshold=400)
            rf = ransac.fit(np.linspace(0,len(self.idx[i][0])-1,len(self.idx[i][0])).reshape(-1,1),self.idx[i][0].reshape(-1,1))
            rf_mask = rf.inlier_mask_
            rf_out_mask = np.logical_not(rf_mask)
            self.idx_final_y.append(self.idx[i][0][rf_mask])
        else:
            self.idx_final_y.append([])

    def main(self,image_dir,out_dir,image_path,width=0):
        ...
        for i in range(img.shape[0]):
            slc = img2[i,:]
            self.idx.append([np.where(slc==1)[0]])
        """
        ...
        tz = time.time()
        pool = Pool()
        results = pool.map(self.ransacmp,range(len(self.idx)))
        pool.close()
        pool.join()
        print(len(results))
...
if __name__=="__main__":
    ...
    mlp = mp()
    ret_fail,mask,bounds = mlp.main(image_dir,out_dir,image_path)
