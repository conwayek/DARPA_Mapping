...
def main(lat3d,lon3d,training,img_shape,out_dir,image_path,image_dir,clue_x,clue_y,redo,lons,lats,clons,clats):
            ...
            key_deltas = np.array([0.125,0.25,0.5],dtype=np.float64)
            done_lat = False
            done_lon = False
            done = False
            if(fity_own == True and fitx_own == True):
                for k in range(len(key_deltas)):
                    ...
                    if(math.isclose(lat3d[2,0],lat3d[2,1]+key_deltas[k],abs_tol=0.03) and done_lat==True):
                        #print(lat_up,lat_low,delta_y_values[i])
                        done_lat = True 
                        good_delta_lat = key_deltas[k]
                        good_delta = key_deltas[k]
                for k in range(len(key_deltas)):
                    if(math.isclose(lon3d[2,0],lon3d[2,1]-key_deltas[k],abs_tol=0.03) and done_lon==False):
                        done_lon = True
                        good_delta_lon = key_deltas[k]
                        if(done_lat == True):
                            if(good_delta == key_deltas[k]):
                                good_delta_lon = key_deltas[k]
                                done=True

            if(fity_own == True and fitx_own == True and done == False):
                print('Checking Fits against each other') 
                print(done_lat,done_lon)
                ...
                if(abs(deltax1/deltay1)>1.3 or abs(deltax1/deltay1)<0.7 or abs(deltax1)<0.5 or abs(deltay1)<0.5 \
                  or abs(deltax1)>25 or abs(deltay1)>25):
                        ...
                        # use lat fit
                        if(done_lat==True and done_lon==False):
                            print('Using Y fit with x points: key-deltas')
                            fitx_own=False                      
                        # use lat fit
                        elif(done_lat==False and done_lon==True):
                            print('Using X fit with y points: key-deltas')
                            fity_own=False 
                        ...
                        if(fity_own == True and fitx_own == True):
                            if(fitx2 == True and fity2==True): 
                                ...
                                if(abs(deltax2)<abs(deltax1) and abs(deltay1)<abs(deltay2) \
                                   and abs(deltax2)>1.5 and abs(deltay1)>1.5):
            ...
            if(fitx_own==False and fity_own==True and lon3d.shape[1]==2):
                ...
                if(lon3d[2,0]!=0):
                    ...
                    for i in range(len(lons)):
                        int_lon = np.floor(lons[i])
                        if(math.isclose(abs(lons[i]-int_lon),0.125,abs_tol=0.03) and done==False):
                             done = True                   
                             lon3d = np.zeros((3,1))
                             lon3d[0,0] = clons[i,1]
                             lon3d[1,0] = clons[i,0]
                             lon3d[2,0] = lons[i]
                        elif(math.isclose(abs(lons[i]-int_lon),0.25,abs_tol=0.03) and done==False):
                             done = True                   
                             lon3d = np.zeros((3,1))
                             lon3d[0,0] = clons[i,1]
                             lon3d[1,0] = clons[i,0]
                    ...
                    print(poptx,dist_vert,dist_horz)
            ...
            if(fity_own==False and fitx_own==True and lat3d.shape[1]==2):
                ...
                if(lat3d[2,0]!=0):
                    ...
                    for i in range(len(lats)):
                        int_lat = np.floor(lats[i])
                        if(math.isclose(abs(lats[i]-int_lat),0.125,abs_tol=0.03) and done==False):
                             done = True                   
                             lat3d = np.zeros((3,1))
                             lat3d[0,0] = clats[i,1]
                             lat3d[1,0] = clats[i,0]
                             lat3d[2,0] = lats[i]
                        elif(math.isclose(abs(lats[i]-int_lat),0.25,abs_tol=0.03) and done==False):
                             done = True                   
                             lat3d = np.zeros((3,1))
                             lat3d[0,0] = clats[i,1]
                             lat3d[1,0] = clats[i,0]
