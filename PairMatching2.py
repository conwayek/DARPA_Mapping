                        ...
                        if(delta_y_values[i]>0 and delta_x_values[j]>0 and delta_y_values[i]<30 and delta_x_values[j]<30):
                            if( (right_point[0] - left_point[0] > 1200 ) and (bot_point[1] - top_point[1] > 1200) and \
                              ...
                              and (clue_y >= (lat_low-0.05)) and (clue_y <= (lat_up+0.05))):
                                #print(lat,lon,delta_y_values[i],delta_x_values[j])
                                if(math.isclose((delta_y_values[i]/delta_x_values[j]),1,abs_tol=\
                                                0.05*(delta_y_values[i]/delta_x_values[j])  )):
                                    if(abs((delta_y_values[i]/delta_x_values[j])-1) <= best_ratio_diff and delta_y_values[i]<smallest_ratio):
...
if __name__=="__main__":
    ...
    lon = np.array([-88. ,   -88.,    -90.  ,  -87. ,   -87.875],dtype=np.float64)
    
    lat = np.array([37.     ,    35.   ,      35.      ,   36.     ,    35.    ,     35., 35.    ,     37.     ,\
                    35.  ,       37.    ,     36.   ,      38., 37.05833333 ,38.    ,     35.   ,      35.  ,\
                    37.    ,     37., 35.   ,      35.     ,    37.  ,       35.     ,    35.    ,     35. , 37.875     ],dtype=np.float64)
    
    clue_x = -88
    clue_y = 37
    
    clon = np.array([[2840.,  601.]  ,[2789. ,7454.], [4528. ,4936.], [5444., 2142.] ,[8292. , 656.]],dtype=np.float64)
    
    
    clat = np.array([[2793.,  642.],
                     [2848., 4600.],
                     [2861., 6750.] ,
                     [2629., 7412.] ,
                     [3056., 7307.] ,
