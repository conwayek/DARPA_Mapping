...
for i in range(nfiles):
    ...
    if(os.path.exists(outname)==False):
        print(outname)
        os.system('python3 DARPA_Color_Temp.py '+str(result_dir)+' '+str(image_dir)+\
                  ' '+str(os.path.basename(file).split('_clue.csv')[0]+'.tif')\
                +' '+str(clue_dir)+\
' '+str(' /scratch/e.conway/DARPA_MAPS/TrainingKeys/ /scratch/e.conway/DARPA_MAPS/TrainingCenters/ /scratch/e.conway/DARPA_MAPS/TrainingBBoxes/ ')\
+' > '+str(os.path.basename(file).split('_clue.csv')[0]+'.out'))
    #"""    
